...
2. Does increasing driver CPU availability alter measured C=1 versus C=16 drain throughput in the high-throughput memory and periodic profiles? Compare 2 and 4 CPU limits with matching `GOMAXPROCS`, using phase-specific driver and broker-container CPU counters. Each retained sampling interval brackets its timed phase; broker-container counts include the controller and, when enabled, the tracer. This evaluates this client path; it does not isolate an implementation-independent broker capacity.
...
Each broker has two CPUs and 4 GiB RAM. Redis's configured memory limit is 2 GB; JetStream's memory-store and file-store limits are each 2 GB. These product limits account for different resources and are not equivalent memory-consumption guarantees. Driver limits are two or four CPUs with 2 GiB RAM, and the corresponding Go runtime setting. Network transport, single-replica scope, and 512-byte payload size follow the original study. Core comparisons use the original highly compressible payload. A payload/compression sensitivity is specified separately.

Before primary collection, the full-duration pilot03 exhausted the original 1 GiB JetStream container during untimed verification. The retained Kubernetes status identifies an OOM kill. The incomplete pilot was stopped, preserved, and excluded. The broker limit was consequently increased uniformly to 4 GiB, with the stated application limits, and validated in a fresh pilot04. This is a prospective resource amendment, not a changed setting within a primary campaign.
...
- Recovery: historical/current JetStream, historical/current Redis XAUTOCLAIM with 10 ms sleep, and current Redis CLAIM; thresholds 250/1000 ms, receipt-based ages 10%/75%, killed/live-unacknowledging victims, and two randomized repetitions. A recorded client-invocation handshake establishes that survivors begin requesting before the reference event. Actual receipt-based age and pre-reference receipts are retained. Redis new-message-only negative controls use both versions and a nominal two-threshold observation budget. Exact observation endpoints, timer-branch selection and receipt timestamps relative to the nominal deadline are retained separately.
...
Current-version always profiles receive matched 1/16-publisher, 15-second trials with and without tracing of both `fsync` and `fdatasync`, with one trial per diagnostic condition in each of the three deployments. Tracing is diagnostic and its overhead is measured by the corresponding untraced conditions. Syscall durations do not establish stable-media behavior or uniquely identify a filesystem cause. Traces use broker wall-clock sample brackets; driver monotonic times from the other node are never treated as broker timestamps. Phase-specific host CPU counters and load averages provide environmental context.
...
The analyzer independently recomputes metrics from raw traces and rejects mismatched counts, duplicate identities, invalid time ordering, stale inputs or missing planned outcomes. Report each deployment's condition means and paired contrasts, the range across deployments, sample counts, failures and raw tail ranks. Throughput contrasts use the ratio of arithmetic condition means over jointly completed randomized blocks. Report the paired cell values and the number of planned, complete and unavailable pairs. Planned contrasts are periodic/memory and always/memory, 16/1 publishers, current/historical versions, 16/1 consumers, and 4/2 driver CPUs, wherever those factors are crossed. Recovery compares killed minus live mean latency within matched policy, threshold, age and block. Diagnostic ratios use their one matched pair per deployment. No interval estimate is inferred from the range of three campaign contrasts. With only three deployment campaigns on shared hardware, do not claim verified population confidence coverage or independent-host uncertainty. New results supplement the original campaign and are never pooled with it as interchangeable replications.
