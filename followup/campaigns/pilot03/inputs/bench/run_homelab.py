...
from campaign_identity import ROOT, check_record, identity, seal, verify_seal, write_new
...
def execute(folder, record):
    verify_seal(folder / "inputs", record)
    ns = record["namespace"]
...
def main():
    ...
    if args.action in ("run", "validation"):
        ...
        try:
            ...
            write_new(folder / "attempt-error.json", {
                **record, "error_type": type(error).__name__, "error": str(error),
                "recorded_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
                "outcome": "incomplete attempt; no automatic replay",
            })
            raise
    else:
