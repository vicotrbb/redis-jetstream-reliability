...
func persistEvents(path string, events [][]Event) (map[string]any, error) {
	...
	g := gzip.NewWriter(f)
	w := csv.NewWriter(g)
	w.Write([]string{"id", "start_ns", "end_ns", "received_ns", "worker", "confirmed", "error", "admitted_ns"})
	count := 0
	for _, group := range events {
		for _, v := range group {
			w.Write([]string{strconv.FormatInt(v.ID, 10), strconv.FormatInt(v.Start, 10), strconv.FormatInt(v.End, 10), strconv.FormatInt(v.Received, 10), strconv.Itoa(v.Worker), strconv.FormatBool(v.Confirmed), v.Error, strconv.FormatInt(v.Admitted, 10)})
			count++
		}
	}
	w.Flush()
	...
	for worker, c := range clients {
		...
		go func(worker int, c *client) {
			...
			for !stopped.Load() {
				...
				id := seq.Add(1) - 1
				b := payload(id)
				v := Event{ID: id, Start: mono(), Worker: worker, Admitted: admitted}
				events[worker] = append(events[worker], v)
				slot := len(events[worker]) - 1
				e := c.publish(t.ID, b)
				if t.Pilot && t.Fault != "" && id == 8 && e == nil {
					if t.Fault == "panic" {
						panic("injected panic after successful append")
					}
					e = fmt.Errorf("injected returned error after successful append")
				}
...
var observationStopped = errors.New("intentional observation stop")
...
func recovery(t Trial, admin *client, r map[string]any) error {
	...
	go func() {
		...
		for {
			select {
			case <-stop:
				done <- result{e: observationStopped, end: mono(), attempts: attempts}
				return
			default:
			}
			attempts++
	...
	case rr := <-done:
		...
		case stamp := <-firstRequest:
			r["survivor_first_request_ns"] = stamp
			done <- rr // Preserve an early receipt if both channels were ready.
		default:
	...
	r["censored"] = rr.m == nil
	r["survivor_error"] = fmt.Sprint(rr.e)
	if rr.e != nil && !errors.Is(rr.e, observationStopped) {
		return rr.e
	}
	if rr.m != nil {
		...
		if e != nil {
			...
			if f, ok := stop["files_gzip"].(map[string]any); ok {
				for name, data := range f {
					...
					decodeErr := json.Unmarshal(j, &encoded)
					if decodeErr == nil && filepath.Base(name) != name {
						decodeErr = fmt.Errorf("invalid diagnostic filename")
					}
					if decodeErr == nil {
						decodeErr = os.WriteFile(filepath.Join(out, name), encoded, 0644)
					}
					if decodeErr != nil {
						r["diagnostic_collection_error"] = decodeErr.Error()
						if r["status"] == "complete" {
							r["status"] = "failed"
							r["error"] = "diagnostic artifact collection failed"
						}
					}
