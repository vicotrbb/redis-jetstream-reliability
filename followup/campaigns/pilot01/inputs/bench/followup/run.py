    ...
    def pod(name, image, cpu, broker=False, engine=""):
        spec = {"nodeSelector": {"kubernetes.io/hostname": "homelab-01" if broker else "homelab-02"},
                ...
                "containers": [{"name": "work", "image": image, "command": ["sh", "-c",
                    ...
                    "resources": {"requests": {"cpu": "100m", "memory": "128Mi"}, "limits": {"cpu": str(cpu), "memory": "1Gi" if broker else "2Gi"}},
        ...
        if broker:
            ...
            spec["initContainers"] = [{"name": "trace-tools", "image": "alpine:3.22", "command": ["sh", "-c", "apk add --no-cache strace && cp /usr/bin/strace /tools/strace && strace -V > /tools/strace-version.txt"], "volumeMounts": [{"name": "tools", "mountPath": "/tools"}]}]
...
def bootstrap(folder, ns):
    pins = json.loads((ROOT / "k8s/image-pins.json").read_text())
    ...
    for pod in ("redis-old", "redis-current", "nats-old", "nats-current"):
        p = execute(["exec", "-n", ns, pod, "--", "sh", "-c", "cat /tools/strace-version.txt; cat /proc/version; df -T /data; cat /proc/self/mountinfo"], capture_output=True, text=True)
...
def main():
    ...
    if args.action == "prepare":
        ...
        snapshot(folder)
        write(folder / "identity.json", {"campaign": args.campaign, "namespace": ns, "context": "homelab", "pilot": args.pilot, "created_utc": now()})
