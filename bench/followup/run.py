...
def bootstrap(folder, ns):
    ...
    if not identity['pilot']:
        assert Path(__file__).read_bytes()==(folder/'inputs/bench/followup/run.py').read_bytes(), 'Bootstrap runner differs from sealed input'
    ...
    distributed={'runner-2':hashlib.sha256(binary.read_bytes()).hexdigest()}
    for pod in ("runner-4", "redis-old", "redis-current", "nats-old", "nats-current"):
        execute(["cp", str(binary), f"{ns}/{pod}:/tools/followup.staging"])
        execute(["exec", "-n", ns, pod, "--", "sh", "-c", "chmod 755 /tools/followup.staging && mv /tools/followup.staging /tools/followup"])
        actual=execute(['exec','-n',ns,pod,'--','sha256sum','/tools/followup'],capture_output=True,text=True).stdout.split()[0]
        assert actual==distributed['runner-2'],'Deployed binary digest differs'
        distributed[pod]=actual
    write(folder/'binary-distribution.json',distributed)
    binary.unlink()
    time.sleep(2)
...
"""
    ...
    with (folder / "progress.jsonl").open("a") as progress:
        for trial in plan:
            ...
            start = now()
            write(folder / 'invocations' / (trial['id']+'-started.json'),
                  {'trial':trial,'started':start,'state':'invocation started; see separate completion receipt'})
            try:
            ...
            assert (p.returncode==0)==(result['status']=='complete'), 'Exit status and retained result disagree'
            assert json.loads(p.stdout)==result, 'Executed result differs from collected result'
    ...
    write(folder/'final-pods.json',json.loads(execute(['get','pods','-n',ns,'-o','json'],capture_output=True,text=True).stdout))
