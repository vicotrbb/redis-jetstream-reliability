...
Release manifests bind the exact source, complete article PDF, analysis and rendered document QA. The builder refuses version reuse and verifies a fresh archive extraction. Hashes establish consistency of supplied bytes, not independent certification of collection time or completeness. Public access and computational reproduction are distinct from an external experimental replication or an independent artifact evaluation.
...
The complete original concurrency matrix retains all thirty conditions, each with ten trials and 16384 messages. It reports trial-mean throughput with nominal bootstrap intervals and distinguishes cycle p99 from common-drain-start-to-receipt p99. Appendix~\ref{app:followup} separately reports the follow-up by deployment, driver quota, concurrency and persistence profile. Machine-readable trial and condition summaries accompany both matrices in the artifact.
...
Stable-storage implication & Conditional storage argument and Proposition~\ref{prop:storage} & Abstract successful-and-honored synchronization contract; no implementation verification\\
Application effect ambiguity & Two-order crash counterexamples and Proposition~\ref{prop:effects} & Separate non-idempotent effect and acknowledgment; not an impossibility result for transactional applications\\
...
\section{Original statistical method and detailed diagnostics}\label{app:history}
These details concern Redis 7.4.2 and NATS Server 2.10.24 in the original accumulated-storage campaign. They remain separate from the fresh-store follow-up in Appendix~\ref{app:followup}.
\input{appendix-history.tex}
\FloatBarrier

\section{Client waits and observation conventions}\label{sec:client-waits}
\input{appendix-client-waits.tex}

\section{Complete follow-up results}\label{app:followup}
\input{appendix-followup.tex}
