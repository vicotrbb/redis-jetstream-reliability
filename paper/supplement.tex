...
\hypersetup{
 pdftitle={Supplementary material: Redis Streams and NATS JetStream reliability study},
...
\title{Supplementary Material for\\An Empirical Evaluation of Message Delivery Reliability and Recovery Characteristics in Redis Streams and NATS JetStream}
\author{Victor Bona}
\date{September 2026}
\begin{document}
\maketitle
\noindent This supplement retains full analytical arguments, the original campaign's detailed diagnostics, and the follow-up's complete condition summaries. Historical numerical observations are unchanged. Statements in the original results concern Redis 7.4.2 and NATS Server 2.10.24 in the original accumulated-storage campaign. They are not descriptions of the subsequent fresh-store deployments, reported separately below and in the main article.

\input{supplement-model.tex}
\section{Original statistical method and attempt history}\label{sec:method}
...
\section{Original campaign results}\label{sec:results}
\subsection{Recorded outcomes and evidence quality}
...
\subsection{RQ1: failure age, reclamation, and observed redelivery}

\begin{table}[htbp]
\centering\small
\input{../data/derived/recovery-table.tex}
...
\subsection{RQ2: concurrency gains and cycle-latency cost}

\begin{table}[htbp]
\centering\small\setlength{\tabcolsep}{4pt}
\input{../data/derived/concurrency-table.tex}
...
\subsection{RQ3: confirmed-publication cost under persistence policies}

\begin{table}[htbp]
\centering\small\setlength{\tabcolsep}{4pt}
\input{../data/derived/durability-table.tex}
...
\input{../data/derived/robustness-table.tex}
...
\begin{table}[htbp]
\centering\small\setlength{\tabcolsep}{4pt}
\input{../data/derived/publication-tail-counts.tex}
...
The original serial-publisher sensitivity gives a different view (the serial table below): always-mode publication averaged 603\,msg/s [591, 614] for Redis and 585\,msg/s [565, 598] for JetStream. The close point estimates do not establish equivalence. This shorter fixed-count workload differs in both publication concurrency and duration, so it cannot isolate batching as the cause of the contrast with the sixteen-publisher results. No syscall or storage-device trace was collected in that original campaign to establish the mechanism. The follow-up's matched-duration publication and syscall diagnostics are a separate dataset.
...
\section{Complete original concurrency matrix}\label{app:matrix}
\begingroup\small\setlength{\tabcolsep}{3.5pt}
\input{../data/derived/full-matrix.tex}
\endgroup
\section{Original serial-publisher sensitivity}\label{app:serial}
\begin{table}[htbp]\centering\small
\input{../data/derived/serial-table.tex}
\caption{Original fixed-count, one-publisher sensitivity.}
\end{table}
\FloatBarrier

\section{Client waits and observation conventions}\label{sec:client-waits}
...
\section{Follow-up condition summaries}
...
\input{../data/derived/followup-publication.tex}
\endgroup
...
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\input{../data/derived/followup-publication-campaigns.tex}
\endgroup
...
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\input{../data/derived/followup-drain-campaigns.tex}
\endgroup
...
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\input{../data/derived/followup-recovery.tex}
\endgroup
...
\begingroup\small
\input{../data/derived/followup-controls.tex}
\endgroup

\subsection{Compression-policy diagnostics}
\begin{table}[!ht]
\centering\footnotesize\setlength{\tabcolsep}{3pt}
\input{../data/derived/followup-compression.tex}
...
\subsection{Failed follow-up outcomes}
...
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\input{../data/derived/followup-failures.tex}
\endgroup

Complete paired contrasts, trial memberships and unavailable-pair counts are in \artifact{followup/derived/statistics.json}. Individual event, telemetry and syscall observations remain in the campaign directories and trial table. The main article presents synchronization diagnostics.
\FloatBarrier
