...
The serial-publisher sensitivity gives a different view (Appendix~\ref{app:serial}): always-mode publication averaged 603\,msg/s [591, 614] for Redis and 585\,msg/s [565, 598] for JetStream. The close point estimates do not establish equivalence. This shorter fixed-count workload differs in both publication concurrency and duration, so it cannot isolate batching as the cause of the contrast with the sixteen-publisher results. No syscall or storage-device trace was collected to establish that mechanism.
