...
func emit(v any) { b, e := json.Marshal(v); must(e); fmt.Println(string(b)) }
...
func connect(profile, name string) (*Client, error) {
	...
	if strings.HasPrefix(profile, "redis-") {
		c.r = redis.NewClient(&redis.Options{Addr: profile + ":6379", PoolSize: 1, MaxRetries: -1, ReadTimeout: 5 * time.Second, WriteTimeout: 5 * time.Second, Protocol: 2})
...
func (c *Client) remove(key string) {
	if c.r != nil {
		must(c.r.Del(ctx, key).Err())
	} else {
		must(c.js.DeleteStream(key))
	}
}
...
func trial(profile string, consumers, publishers, count int, key, out string) (map[string]any, error) {
...
func campaign(mode, out string) {
	...
	serial := 0
	writeJSON(filepath.Join(out, "runtime.json"), map[string]any{"utc_start": time.Now().UTC(), "go": runtime.Version(), "gomaxprocs": runtime.GOMAXPROCS(0), "cpuinfo": readFile("/proc/cpuinfo"), "kernel": readFile("/proc/version"), "mode": mode, "seed": 20260923})
	for block := 0; block < blocks; block++ {
		...
		for _, cell := range cells {
			serial++
			key := fmt.Sprintf("T%s%03d", mode, serial)
			_, e := trial(cell.p, cell.c, pubs, 256, key+"warm", "")
			if e != nil {
				v := map[string]any{"kind": "error", "block": block, "key": key, "profile": cell.p, "stage": "warmup", "error": e.Error()}
				must(enc.Encode(v))
				emit(v)
				panic(e)
			}
			...
			if e != nil {
				v = map[string]any{"kind": "error", "block": block, "key": key, "profile": cell.p, "error": e.Error()}
				must(enc.Encode(v))
				emit(v)
				panic(e)
			}
...
func recoverOne(profile string, delta time.Duration, age float64, poll time.Duration, seed int64, key string, noReclaim bool) map[string]any {
	...
	go func() {
		...
		for {
			select {
			case <-stop:
				result <- Result{nil, errors.New("censored"), attempts}
				return
			default:
			}
			attempts++
			if survivor.r != nil && !noReclaim {
				...
				ts := mono()
				if e != nil {
					result <- Result{nil, e, attempts}
					return
				}
				if len(msgs) > 0 {
					m := msgs[0]
					result <- Result{&Message{id: int(binary.BigEndian.Uint64([]byte(m.Values["p"].(string)))), redisID: m.ID, received: ts}, nil, attempts}
					return
				}
				time.Sleep(poll)
			} else {
